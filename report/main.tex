...
Due to computational constraints, a convenience sample stratified by drug category (5 records per drug) was utilized for preliminary evaluation. Furthermore, it was identified that the provided annotation files for the ECGRDVQ dataset primarily target T-wave morphology rather than QRS complexes. 
...
To benchmark the proposed modular pipeline, its performance was compared against the industry-standard Pan-Tompkins algorithm~\cite{pan_real-time_1985}. The NeuroKit2 implementation of Pan-Tompkins was utilized, which applies its own internal 5--15\,Hz bandpass filter to the raw ECG signal before the derivative and squaring steps; this preprocessing is equivalent to a rudimentary noise suppression stage but does not perform wavelet decomposition. Because independent ground-truth annotations were unavailable, a relative benchmarking strategy was adopted. The performance metric used was the absolute BPM difference between the proposed method and the Pan-Tompkins baseline across the convenience sample.

As shown in Table~\ref{tab:benchmark} and Figure~\ref{fig:benchmarking}, a severe discrepancy was observed. The baseline Pan-Tompkins algorithm yielded an average absolute difference of 59.76 BPM compared to the proposed method, frequently reporting heart rates nearly double the true physiological rate (e.g., 132 BPM vs. 66 BPM).

\begin{table}[h!]
\centering
\caption{Heart rate benchmarking demonstrating T-wave over-sensing in the Pan-Tompkins baseline compared to the proposed method on a 10-second window.}
\label{tab:benchmark}
    \begin{tabular}{llccc}
        \toprule
...
This massive difference is due to \textit{T-wave over-sensing} by the baseline algorithm. Pharmacological datasets frequently feature altered ventricular repolarization, resulting in prominent or abnormal T-waves. While the internal bandpass filter of the standard Pan-Tompkins method handles standard Holter noise well, it is highly susceptible to falsely identifying these massive, drug-induced T-waves as independent QRS complexes. 

Conversely, the proposed Wavelet and K-Means pipeline proved highly resilient to this artifact. By utilizing a 9-level wavelet decomposition and relying on absolute gradient slope clustered via K-Means, the algorithm successfully isolated the high-frequency true R-peaks from the lower-frequency, drug-altered T-waves, making it significantly more robust for the analysis of complex pharmacological ECG data.
...
Furthermore, the adaptation of the pipeline to the ECGRDVQ database demonstrated the algorithm's generalizability to complex pharmacological trials. In a direct baseline comparison, the proposed method proved highly resilient to drug-induced T-wave over-sensing, achieving accurate heart rate extraction where the classical Pan-Tompkins algorithm suffered from severe double-counting (yielding a mean absolute BPM difference of 59.76). This underscores the superiority and robustness of the absolute-slope and K-Means approach over traditional amplitude-threshold methods in real-world clinical applications.

While the algorithm achieved exceptional performance on the Ranolazine subset, preliminary testing on the Dofetilide subset revealed a specific vulnerability to drug-induced repolarization abnormalities. Dofetilide, a Class III antiarrhythmic, frequently induces severe QT prolongation and massive, steep T-waves. In these specific cases, the absolute slope of the abnormal T-wave occasionally rivals that of the QRS complex, causing the K-Means clustering to falsely classify the T-wave as a QRS region. Furthermore, because of the prolonged QT interval, these prominent T-waves often fall outside the physiological refractory shield of 250ms, resulting in T-wave over-sensing. Future iterations of this pipeline could address this by dynamically scaling the refractory period based on the local RR-interval, or by implementing an adaptive amplitude threshold within the high-slope K-Means cluster to distinguish between the R-peak and the T-wave.
